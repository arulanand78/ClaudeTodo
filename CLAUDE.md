...
- `App.jsx` — single page: input + Add, list of todos (checkbox + delete). All mutations are **optimistic** (local state updated immediately), then **reconcile on success**, or **rollback + refetch on error**. It also does a `GET` on mount so a reload reflects server state.
...
|---|---|---|---|---|
| POST | `/todos` | `{ "title": "..." }` | `201` `{ id, title, completed, created_at }` | `400` bad title |
...
`id` is a UUID; `created_at` is an ISO-8601 UTC timestamp set by the server. Error shape is always `{ "detail": "..." }`.